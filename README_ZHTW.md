...
> **`e` 是封裝後的 `EntityId`，不是欄位索引。** `e` 可直接用於實體操作（`destroyEntity`、`hasComponent`、`getComponent`、command buffer）。要索引 SoA 欄位，請以 `getEntityIndex(e)` 轉換（每個實體在 callback 開頭轉一次）：`pos.x[getEntityIndex(e)]`。封裝後的 id 只有在 slot 尚未被回收（世代 0）時才等於索引；任何 `destroyEntity` 回收 slot 之後，`e !== getEntityIndex(e)`，以 `e` 索引欄位會讀到錯誤的 slot（越界 → `undefined`/`NaN`）。
...
const movementSystem = (w, dt) => {
  forEachEntity(w, movers, (e, pos, vel) => {
    const i = getEntityIndex(e)   // 封裝後的 EntityId → 欄位索引
    pos.x[i] += vel.x[i] * dt
    pos.y[i] += vel.y[i] * dt
  })
  return w
...
`forEachEntity` callback 的第一個參數是**封裝後的 `EntityId`**（索引 + 世代），不是欄位索引。請在 callback 開頭以 `const i = getEntityIndex(e)` 轉換一次，並以 `i` 索引每個 SoA 欄位；封裝後的 `e` 則傳給實體操作（`destroyEntity`、`hasComponent`、command buffer）。關於 slot 回收後為何 `e !== i`，見[核心概念 → 實體](#核心概念)。
...
const damageSystem = (world) => {
  ...
  withCommandBuffer(world, (cb) => {
    forEachEntity(world, dying, (e, health) => {
      if (health.hp[getEntityIndex(e)] <= 0) cb.destroy(e)   // 索引欄位用 index，排入佇列用封裝後的 `e`
    })
  })  // 區塊結束時自動 flush
  return world
}
...
- SoA 欄位是以實體**索引**（永遠落在 `[0, getWorldCapacity(world))`）索引的 TypedArray，切勿直接用封裝後的 `eid` —— slot 被回收後兩者就不同。預設佈局（`indexBits: 24`）下索引即 `getEntityIndex(eid)`；若以非預設 `indexBits` 建立 world，`getEntityIndex` 使用預設 24-bit mask、並非欄位索引 —— 此時請改用 `EntityRef` + `deref`。
...
- **`forEachEntity` 的 `e` 是封裝後的 `EntityId`，不是欄位索引。** `e` 用於實體操作（`destroyEntity(w, e)`、`hasComponent`、`getComponent`、command buffer）。索引 SoA 欄位請用 `getEntityIndex(e)`：`pos.x[getEntityIndex(e)]`，並在 callback 開頭轉一次（`const i = getEntityIndex(e)`）。以原始 `e` 索引欄位只在尚無 slot 被回收（世代 0）時可行；任何 `destroyEntity` 之後就會讀到錯誤/越界的 slot。
- **tag 也會佔用一個 callback 參數位。** `defineQuery([Tag, Position])` 會以 `(e, true, posCols)` 呼叫 callback —— tag 以字面值 `true` 傳入。請把 tag 放到陣列**最後**（`defineQuery([Position, Tag])`），讓資料欄位排在前面；或接受並忽略尾端的 `true`。對 tag 在前的查詢寫 `(e, pos)` 會讓 `pos === true`，`pos.x` 會丟例外。
