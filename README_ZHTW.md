...
> **狀態：0.5.7 — 實驗版（0.5.x 系列）。** `STABILITY.md` 中載明的 API 表面在 0.x 系列內承諾穩定，但仍可能微調。1.0 穩定凍結會在收集社群回饋後執行。
...
黃金法則：**迭代中要新增或移除元件，請透過 command buffer** —— 切勿在迭代中直接修改該實體的元件集合。新增或移除元件會在迭代途中改變 archetype 歸屬，可能讓某些實體被跳過或被處理兩次。迭代中呼叫 `destroyEntity` **是安全的**（迭代會每一步重讀存活列數，因此 callback 絕不會收到保留的 eid 0、也不會收到已 destroy 的實體），但有兩個注意事項：被換位填入已釋放列的倖存者會**延到下一個 pass**（本 pass 不會再被走訪），且 destroy 之後的迭代順序不保證。元件變更請用 command buffer 延後執行：
